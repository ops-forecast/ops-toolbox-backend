...
app.post('/shopify/graphql', async (req, res) => {
  try {
    // Load saved shopify config
    const cfgPath = filePath('shopify-config');
    if (!fs.existsSync(cfgPath)) {
      return res.status(401).json({ error: 'Shopify not configured. Please set up your store credentials first.' });
    }
    const cfg = JSON.parse(fs.readFileSync(cfgPath, 'utf8')).value;
    const { store, token, apiVersion = '2024-10' } = cfg;
